...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['user_id'])) {
    ...
    class UserController {
        ...
        public function confirmUser($userId) {
            ...
            // Rediriger vers une page de confirmation après la validation
            header("Location: ../view/AdminDashboard.php"); // Rediriger après la validation
            exit();  // Toujours appeler exit après une redirection
        }
    }

    // Créer une instance du contrôleur et appeler la méthode confirmUser avec l'ID utilisateur
    $controller = new UserController();
    $controller->confirmUser($user_id); // Passer l'ID utilisateur à la méthode confirmUser
}
